...
export function useSimulator({ nodes, edges, onComplete }) {
  ...
  const advance = useCallback(
    (respectPause = true) => {
      ...
      if (step.type === "end") {
        setSimState((s) => ({ ...s, isPlaying: true, currentNodeId: null, varState }));
        onComplete?.(prev.stepCount);
        return;
      }

      if (step.type === "branch") {
        setSimState((s) => ({ ...s, awaitingBranch: { options: step.options }, varState }));
        return;
      }
  ...
  const start = useCallback(() => {
    ...
    if (!entry) return;
    setSimState({ ...emptySimState(), currentNodeId: entry.id, isPlaying: true });
    scheduleAutoStep();
  }, [scheduleAutoStep]);
  ...
  const chooseBranch = useCallback(
    (targetId) => {
      const prev = simStateRef.current;
      const chosen = prev.awaitingBranch?.options?.find((o) => o.targetId === targetId);
      if (!chosen) return;
      setSimState((s) => ({
        ...s,
